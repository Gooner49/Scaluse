    ...
    def main(args: Array[String]) : Unit =
    {
        ...
        .load()
            ...
            val windowedCounts = df1.groupBy(
                 ...
                 .select(col("AccountID").cast("string").as("key"), concat(col("AccountID"),lit(";"),col("SumAmnt")).cast("string").as("value"))
                 //df.select(col("salary").cast("int").as("salary"))
                 //.select("AccountID"  ,"SumAmnt")
                .writeStream
                .format("kafka")
                .option("topic", "ztopia_1")
                .option("kafka.bootstrap.servers", "cxln4.c.thelab-240901.internal:6667")
                .option("checkpointLocation", "/user/sankrrs889440/checkpoint_kafka/")
                .outputMode("update")
                .start()
                .awaitTermination()
